...
def output(mlmodel, sample_buffer):
      # feed in sample_buffer from the main loop, get it through args
    sensor_data = {
        "posture_quality": ml_predict(mlmodel,sample_buffer).item(),
...
publish_thread = threading.Thread(target = output(model, sampler_buffer))
