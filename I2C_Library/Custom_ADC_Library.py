...
class flex_adc(object):
    ...
    def __init__(self, gain_set=4, data_rate=250, address=DEFAULT_SLAVE_ADDRESS, i2c=None, **kwargs):
    ...
    def get_sample(self, channel=0, data_rate=None): 
        ...
        with smbus2.SMBus(1) as bus:
            ...
            #converted=flex_adc.bytes2signed(self, read_result.buf[0][0], read_result.buf[1][0])
            #print(converted)
            #or one read then one write
            return read_result.buf[0][0] << 8 | read_result.buf[1][0]
